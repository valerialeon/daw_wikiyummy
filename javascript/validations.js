...
let regex_date = /^(0[1-9]|1[0-2])\/(0[1-9]|1\d|2\d|3[01])\/(0[1-9]|1[1-9]|2[1-9])$/;
const comunas = [[''], ['Iquique', 'Alto Hospicio', 'Pozo Almonte', 'Camiña', 'Colchane', 'Huara', 'Pica'],
               ['Antofagasta', 'Mejillones', 'Sierra Gorda', 'Taltal', 'Calama', 'Ollagüe', 'San Pedro de Atacama', 'Tocopilla', 'María Elena'],
                ...
                ['Talca', 'Constución', 'Curepto', 'Empedrado', 'Maule', 'Pelarco', 'Pencahue', 'Río Claro', 'San Clemente', 'San Rafael', 'Cauquenes', 'Chanco', 'Pelluhue', 'Curicó', 'Hualañé', 'Licantén', 'Molina', 'Rauco', 'Romeral', 'Sagrada Familia', 'Teno', 'Vichuquén', 'Linares', 'Colbún', 'Longaví', 'Parral', 'ReVro', 'San Javier', 'Villa Alegre', 'Yerbas Buenas'],
...
//Funciones para setear fecha y horas iniciales
function actualDate(){
    let actual = new Date();
    let actualDate = actual.getFullYear() + "-" + actual.getMonth() + 1 +
                    "-" + actual.getDate() + " " + actual.getHours() + ":"+
                    actual.getMinutes();
    document.getElementById("dia-hora-inicio").setAttribute("value", actualDate);

}
function actualEndDate(){
    let actual = new Date();
    let actualDate = actual.getFullYear() + "-" + actual.getMonth() + 1 +
                    "-" + actual.getDate() + " " + actual.getHours() +3+ ":"+
                    actual.getMinutes();
    document.getElementById("dia-hora-inicio").setAttribute("value", actualDate);
...
function validateDate(){
    let value_in = document.getElementById("dia-hora-inicio").getAttribute("value");
    let value_end = document.getElementById("dia-hora-termino").getAttribute("value");
    let validate = value_in.length<=20||regex_date.test(value_in);
    validate = validate || value_end.length<=20||regex_date.test(value_end);
    return validate
}
...
function Comunas(){
    let region_str = document.getElementById("region").value;
    let region_int = region_str.toString();
    const com = comunas[region_int];
    let options = "";
    for (let i = 0; i<com.length; i++){
        options += "<option value='" + i.toString()+ "' >" + com[i] + "</option>" ;
